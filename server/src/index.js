...
const client = new Pool({
  ...
  port: process.env.PGPORT || 5432, // put the port here
  database: process.env.PGDB || "movies-app",
...
app.get("/movie/:id", async (req, res) => {
  try {
    ...
    const result = await client.query("SELECT * FROM movies WHERE id = $1", [
      id,
    ]);
    const movie = result.rows[0];

    movie.rating = movie.rating_result
      ? movie.rating_result / movie.rating_count
      : null;
...
app.post("/movie", async (req, res) => {
  try {
    ...
    if (duplicate.rows[0]) {
      return res.status(500).send(errorMessages.TITLE_DUPLICATE);
    }

    if (data.year > todayDate.getFullYear() || data.year < 1000) {
      return res.status(500).send(errorMessages.INCORRECT_YEAR);
    } else {
      ...
      insertedRow.rows[0].rating =
        insertedRow.rows[0].rating_result / insertedRow.rows[0].rating_count;
...
app.put("/movie/:id", async (req, res) => {
  ...
  if (data.year > todayDate.getFullYear() || data.year < 1000) {
    ...
    const result = await client.query(
      "UPDATE movies SET title=$1, director=$2, genre=$3, year=$4, description=$5, image_url=$6 WHERE id = $7",
      [
        data.title,
        data.director,
        data.genre,
...
app.get("/movie/:id/rate", async (req, res) => {
  ...
  if (!movie.rows[0]) {
    return res.status(500).send(errorMessages.ELEMENT_NOT_EXIST);
  } else {
    try {
      const newMovie = {
        id: id,
...
app.patch("/movie/:id/rate", async (req, res) => {
  ...
  if (!movie.rows[0]) {
    ...
    if (score < 1 || score > 5) {
      return res.status(500).send(errorMessages.INCORRECT_RATING_SCORE);
    } else {
      const movieFromDb = movie.rows[0];
      try {
        await client.query(
          "UPDATE movies SET rating_count=$1, rating_result=$2 WHERE id = $3",
          [movieFromDb.rating_count + 1, movieFromDb.rating_result + score, id]
        );
        movieFromDb.rating_count = movieFromDb.rating_count + 1;
        movieFromDb.rating =
          (movieFromDb.rating_result + score) / movieFromDb.rating_count;
